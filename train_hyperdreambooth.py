...
def save_model_card(
    ...
    # [TODO] design model card for hypernetwork
    img_str = ""
...
def main(args):
    ...
    if args.pre_opt_weight_path != '':
        ...
        for lora, aux_seed in zip(unet_lora_linear_layers, aux_seed):
            lora.aux_seed = aux_seed
        logger.info(f"Pre-optimized weights loaded from {args.pre_opt_weight_path}. total aux seed: {weight['aux_seed'].size(0)}")
    ...
    if accelerator.is_main_process:
        ...
        # [TODO] add hypernetwork into pipeline
